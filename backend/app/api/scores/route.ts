...
import { recentScores, submitScore } from "@/lib/store";
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof body.score !== "number" || typeof body.level !== "number") {
      return error("level and score are required numbers");
    }
    ...
    return json(
      {
        ...
        player: {
          ...result.player,
          winRate: winRate(result.player.stats),
        },
      },
      201
    );
